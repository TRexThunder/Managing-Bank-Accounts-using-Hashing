...
void merge4(std::vector<int> &v, int l, int m, int r) {
    int s1=m-l+1, s2=r-m;
...
void msort4(std::vector<int> &v, int x, int y) {
    if (x<y) {
        int z=x+(y-x)/2;
        msort4(v, x, z);
        msort4(v, z+1, y);
...
void CubicProbing::createAccount(std::string id, int count) {
    ...
    int hid=hash(id);
    while (true) {
        int current=(hid+counter*counter*counter)%142871;
...
int CubicProbing::getBalance(std::string id) {
    ...
    int hid=hash(id);
    for (long long i=0; i<142871; i++) {
        int h=(hid+i*i*i)%142871;
